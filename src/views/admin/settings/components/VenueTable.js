...
export default function VenueTable(props) {
  ...
  const handleClick = () => {
    Swal.fire({
      title: "Add Venue",
      html: `
      <input id="name" class="swal2-input" placeholder="Venue Name" autocapitalize="off">
    `,
      ...
      if (result.isConfirmed) {
        const venueName = document.getElementById('name').value.trim();
        if (venueName) {
          // Make the POST request to add the department
          request("POST", "/api/v1/admin/addVenue", { name: venueName })
            .then((response) => {
            ...
            .catch((error) => {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to add venue. Please try again later.'
              });
            });
        } else {
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Venue name cannot be empty!'
          });
        }
      }
    });
  ...
  const handleSubmit = (id) => {
    ...
    request("PUT", `/api/v1/admin/venue/update/${id}`, { name: newName })
      .then((response) => {
        Swal.fire({
          icon: 'success',
          title: 'Success',
      ...
      .catch((error) => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Failed to update venue. Please try again later.'
        });
      });
  ...
  const handleDeleteDepartment = (id) => {
    request("PUT", `/api/v1/admin/venue/delete/${id}`, {} )
      .then((response) => {
        Swal.fire({
          icon: 'success',
          title: 'Success',
