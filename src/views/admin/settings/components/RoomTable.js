...
export default function RoomTable(props) {
  ...
  const handleClick = () => {
    ...
    `,
      ...
      if (result.isConfirmed) {
        const roomName = document.getElementById('name').value.trim();
        if (roomName) {
          // Make the POST request to add the department
          request("POST", "/api/v1/admin/addRoom", { name: roomName })
            .then((response) => {
  ...
  const handleSubmit = (id) => {
    ...
    request("PUT", `/api/v1/admin/room/update/${id}`, { name: newName })
      .then((response) => {
        Swal.fire({
          icon: 'success',
          title: 'Success',
  ...
  const handleDeleteDepartment = (id) => {
    request("PUT", `/api/v1/admin/room/delete/${id}`, {} )
      .then((response) => {
        Swal.fire({
          icon: 'success',
          title: 'Success',
